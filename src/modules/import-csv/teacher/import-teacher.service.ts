...
export class ImportTeacherService {
        ...
        for (const { index, row } of batch) {
            ...
            if (dto.teacherEmail) {
                ...
                if (existingEmails.has(emailLower)) {
                    warningMessages.push(`อีเมล ${dto.teacherEmail} มีอยู่ในระบบแล้ว (จะข้ามการบันทึก)`);
                    isDuplicate = true;
    ...
    async validateTeacherData(instId: number, instType: string, buffer: Buffer) {
        ...
        const [learningAreas, existingUsers] = await Promise.all([
            this.learningAreaRepo.find({ where: { inst_id: instId, flag_valid: true } }),
        ...
        rawData.forEach((row, index) => {
            ...
            if (dto.teacherId && !firstOccurrences.code.has(dto.teacherId)) firstOccurrences.code.set(dto.teacherId, index);
        });
        ...
        const validatedData = await processBatchesParallel(
            batches,
            (batch) => this.validateBatch(batch, learningAreas, instId, existingEmails, existingCodes, firstOccurrences),
            IMPORT_MAX_CONCURRENT_BATCHES
        );
    ...
    private async saveBatch(
        ...
        for (const row of batch) {
            ...
            const result = await queryRunner.manager.query(insertUserQuery, userValues);
    ...
    async saveTeacherData(instId: number, instType: string, buffer: Buffer, validationToken?: string) {
        if (!validationToken) {
