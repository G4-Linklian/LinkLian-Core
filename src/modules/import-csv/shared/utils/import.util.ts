...
export function verifyValidationToken(
    ...
    try {
        ...
        if (payload.type !== expectedType) {
            throw new UnauthorizedException(`Token ไม่ใช่สำหรับนำเข้าข้อมูล${getTypeLabel(expectedType)}`);
        }
        ...
        if (payload.dataHash !== currentDataHash) {
            throw new BadRequestException('ข้อมูลถูกเปลี่ยนแปลงหลังจาก validate กรุณา validate ใหม่');
        }
