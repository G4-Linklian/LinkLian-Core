...
export class ImportEnrollmentService {
    ...
    const result = {
      students: new Map(
        studentUsers
          .filter((s) => s.code)
          .map(
            (s) => [s.code!.toLowerCase(), s.user_sys_id] as [string, number],
          ),
      ),
      existingEnrollments: new Set<number>(
        existingEnrollments.map((e) => e.student_id as number),
      ),
    };
  ...
  private async validateBatch(
    ...
    for (const { index, row } of batch) {
      ...
      const dto = plainToInstance(ImportEnrollmentDto, row, {
      ...
      if (studentId) {
        if (preFetchedData.existingEnrollments.has(studentId)) {
          warningMessages.push(
            `นักเรียน "${dto.studentCode}" ลงทะเบียนในกลุ่มเรียนนี้แล้ว (จะข้ามการบันทึก)`,
          );
          isDuplicate = true;
  ...
  async validateEnrollmentData(
    ...
    if (!section || section.length === 0) {
      throw new NotFoundException(
        `Section ID ${sectionId} ไม่พบในระบบหรือไม่ตรงกับสถาบัน`,
      );
    }
  ...
  private async saveBatch(
    ...
    for (const row of batch) {
      ...
      const studentId = preFetchedData.students.get(
        dto.studentCode.toLowerCase(),
      );
      if (!studentId) {
        throw new NotFoundException(
          `รหัสนักเรียน "${dto.studentCode}" ไม่พบในระบบ`,
        );
      }
      ...
      const insertEnrollmentQuery = `
                ...
                VALUES ($1, $2, true, NOW())
            `;
      await queryRunner.manager.query(insertEnrollmentQuery, [
        sectionId,
        studentId,
      ]);
  ...
  async saveEnrollmentData(
    ...
    this.logger.debug(`Saving enrollments`, 'SaveEnrollmentData', {
      instId,
      sectionId,
    });
