...
import {
  ...
  ProgramType,
  TreeType,
} from '../../program/entities/program.entity';
import { Institution } from '../../institution/entities/institution.entity';
import {
  ImportSchoolProgramDto,
  ImportUniProgramDto,
} from './dto/import-program.dto';
import {
  ValidatedRow,
  calculateDataHash,
  chunkArray,
...
export class ImportProgramService {
  ...
  private async validateSchoolBatch(
    ...
    for (const { index, row } of batch) {
      ...
      const dto = plainToInstance(ImportSchoolProgramDto, row, {
      ...
      if (existingCombinations.has(combinationKey)) {
        warningMessages.push(
          `แผนการเรียน "${dto.programName}" + ห้อง "${dto.className}" มีอยู่ในระบบแล้ว (จะข้ามการบันทึก)`,
        );
        isDuplicate = true;
  ...
  private async validateUniBatch(
    ...
    for (const { index, row } of batch) {
      ...
      const dto = plainToInstance(ImportUniProgramDto, row, {
        excludeExtraneousValues: true,
      });
      const rowErrors = await validate(dto);
      if (rowErrors.length > 0) {
      ...
      if (existingCombinations.has(combinationKey)) {
        warningMessages.push(
          `คณะ "${dto.faculty}" + ภาค "${dto.department}" + สาขา "${dto.major}" มีอยู่ในระบบแล้ว (จะข้ามการบันทึก)`,
        );
        isDuplicate = true;
      } else if (firstOccurrences.get(combinationKey) !== index) {
  ...
  async validateProgramData(instId: number, instType: string, buffer: Buffer) {
    ...
    this.logger.debug(`Total programs found`, 'validateProgramData', {
      inst_id: instId,
      found: existingPrograms.length,
    });
    this.logger.debug(
    ...
    if (isSchool) {
      const studyPlans = existingPrograms.filter(
        (p) =>
          p.program_type?.toLowerCase() ===
          ProgramType.STUDY_PLAN.toLowerCase(),
      );
      const classes = existingPrograms.filter(
        (p) =>
          p.program_type?.toLowerCase() === ProgramType.CLASS.toLowerCase(),
      );
      this.logger.debug(
        `Found ${studyPlans.length} study plans, ${classes.length} classes`,
        'validateProgramData',
      );

      classes.forEach((cls) => {
        const parent = studyPlans.find(
          (sp) => sp.program_id === Number(cls.parent_id),
        );
        if (parent) {
          existingCombinations.add(
            `${parent.program_name.toLowerCase()}|${cls.program_name.toLowerCase()}`,
          );
        } else {
          this.logger.debug(
            `Class "${cls.program_name}" has parent_id ${cls.parent_id} but no matching study plan found`,
            'validateProgramData',
          );
        }
      });
    } else {
      const faculties = existingPrograms.filter(
        (p) =>
          p.program_type?.toLowerCase() === ProgramType.FACULTY.toLowerCase(),
      );
      const depts = existingPrograms.filter(
        (p) =>
          p.program_type?.toLowerCase() ===
          ProgramType.DEPARTMENT.toLowerCase(),
      );
      existingPrograms
        .filter(
          (p) =>
            p.program_type?.toLowerCase() === ProgramType.MAJOR.toLowerCase(),
        )
        .forEach((major) => {
          const dept = depts.find(
            (d) => d.program_id === Number(major.parent_id),
          );
          const faculty = dept
            ? faculties.find((f) => f.program_id === Number(dept.parent_id))
            : null;
          if (faculty && dept)
            existingCombinations.add(
              `${faculty.program_name.toLowerCase()}|${dept.program_name.toLowerCase()}|${major.program_name.toLowerCase()}`,
            );
        });
      this.logger.debug(
        `Found ${faculties.length} faculties, ${depts.length} departments and ${existingCombinations.size} existing combinations`,
        'validateProgramData',
      );
    }
    ...
    const validatedData = await processBatchesParallel(
      ...
      (batch) =>
        isSchool
          ? this.validateSchoolBatch(
              batch,
              existingCombinations,
              firstOccurrences,
            )
          : this.validateUniBatch(
              batch,
              existingCombinations,
              firstOccurrences,
            ),
    ...
    for (const row of batch) {
      ...
      if (!studyPlanId) {
        const insertStudyPlanQuery = `
                ...
                `;
        const studyPlanResult = await queryRunner.manager.query(
          insertStudyPlanQuery,
          [instId, studyPlanName, ProgramType.STUDY_PLAN, TreeType.ROOT],
        );
        ...
        if (!studyPlanId) {
          const existingQuery = `
                    ...
                    `;
          const existingResult = await queryRunner.manager.query(
            existingQuery,
          ...
          if (!studyPlanId) {
            throw new Error(
              `Failed to create or find study plan: ${studyPlanName}`,
            );
          }
        } else {
          savedCount++;
    ...
    for (const row of batch) {
      ...
      if (!facultyId) {
        const insertFacultyQuery = `
                    ...
                    RETURNING program_id
                `;
        const facultyResult = await queryRunner.manager.query(
          insertFacultyQuery,
          [instId, facultyName, ProgramType.FACULTY, TreeType.ROOT],
        );
  ...
  async saveProgramData(
    ...
    this.logger.debug(
      `Saving programs for inst_id: ${instId}, inst_type: ${instType}`,
      'saveProgramData',
    );
    ...
    this.logger.debug(
      `Valid token - proceeding to save ${payload.validCount} records (${payload.duplicateCount || 0} duplicates will be skipped)`,
      'saveProgramData',
    );
    ...
    try {
      ...
      if (isSchool) {
        ...
        const existingFaculties = new Map<string, number>(
          existingPrograms
            .filter(
              (p) =>
                p.program_type?.toLowerCase() ===
                ProgramType.FACULTY.toLowerCase(),
            )
            .map((p) => [p.program_name.toLowerCase(), p.program_id]),
        );
        ...
        const departments = existingPrograms.filter(
          (p) =>
            p.program_type?.toLowerCase() ===
            ProgramType.DEPARTMENT.toLowerCase(),
        );
        ...
        for (const dept of departments) {
          const faculty = faculties.find(
            (f) => f.program_id === Number(dept.parent_id),
          );
          if (faculty) {
        ...
        const existingCombinations = new Set<string>();
        const majors = existingPrograms.filter(
          (p) =>
            p.program_type?.toLowerCase() === ProgramType.MAJOR.toLowerCase(),
        );
