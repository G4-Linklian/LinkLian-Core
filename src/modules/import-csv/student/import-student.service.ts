...
//   | typeof ImportSchoolStudentDto
//   | typeof ImportUniStudentDto;
...
export class ImportStudentService {
  ...
  private async validateBatch(
    ...
    for (const { index, row } of batch) {
      ...
      const studentDto = plainToInstance(DtoClass as any, row, {
        excludeExtraneousValues: true,
      }) as unknown as StudentDtoType;
      ...
      const eduLevel = eduLevels.find(
        (el) => el.level_name === studentDto.eduLevel,
      );
      if (!eduLevel) {
        errorMessages.push(
          `ระดับชั้นที่ระบุไม่ถูกต้อง: ${studentDto.eduLevel}`,
        );
      }
      ...
      const possibleClassrooms = programs.filter(
        (p) =>
          p.program_name === classroomName &&
          (p.program_type === ProgramType.CLASS ||
            p.program_type === ProgramType.MAJOR) &&
          p.flag_valid,
      );
        ...
        classroom = possibleClassrooms.find((c) =>
          programs.some(
            (p) =>
              ...
              p.program_type ===
                (isSchool ? ProgramType.STUDY_PLAN : ProgramType.DEPARTMENT) &&
              p.flag_valid,
          ),
        );
      }
      ...
      if (possibleClassrooms.length === 0) {
        ...
        errorMessages.push(
          `ห้องเรียน "${classroomName}" ไม่สอดคล้องกับ${isSchool ? 'แผนการเรียน' : 'ภาค'} "${parentProgramName}"`,
        );
      }
      ...
      if (studentDto.studentEmail) {
        ...
        if (existingEmails.has(emailLower)) {
          warningMessages.push(
            `อีเมล ${studentDto.studentEmail} มีอยู่ในระบบแล้ว (จะข้ามการบันทึก)`,
          );
          isDuplicate = true;
        } else if (firstOccurrences.email.get(emailLower) !== index) {
          errorMessages.push(
            `อีเมล ${studentDto.studentEmail} ซ้ำกันภายในไฟล์`,
          );
        }
      }
      ...
      if (studentDto.studentId) {
        if (existingCodes.has(studentDto.studentId)) {
          warningMessages.push(
            `รหัสนักเรียน ${studentDto.studentId} มีอยู่ในระบบแล้ว (จะข้ามการบันทึก)`,
          );
          isDuplicate = true;
        } else if (firstOccurrences.code.get(studentDto.studentId) !== index) {
          errorMessages.push(
            `รหัสนักเรียน ${studentDto.studentId} ซ้ำกันภายในไฟล์`,
          );
        }
      }
  ...
  async validateStudentData(instId: number, instType: string, buffer: Buffer) {
    ...
    rawData.forEach((row, index) => {
      ...
      if (
        ...
        !firstOccurrences.code.has(studentDto.studentId)
      )
        firstOccurrences.code.set(studentDto.studentId, index);
    });
  ...
  private async saveBatch(
    ...
    for (const row of batch) {
      ...
      if (
        ...
        this.logger.debug(
          `Skipping duplicate: ${studentDto.studentEmail || studentDto.studentId}`,
          'StudentSaveBatch',
        );
        skippedCount++;
        continue;
      ...
      if (!eduLevel) {
        throw new NotFoundException(
          `ระดับชั้น ${studentDto.eduLevel} ไม่พบในระบบ`,
        );
      }
      ...
      const rawStatus =
        studentDto.studentStatus?.trim().toLowerCase() || 'active';
      ...
      const studentProgramQuery = `
                ...
                VALUES ($1, $2, true)
            `;
      await queryRunner.manager.query(studentProgramQuery, [
        userSysId,
  ...
  async saveStudentData(
    ...
    this.logger.debug(
      `Saving students for inst_id: ${instId}, inst_type: ${instType}`,
      'StudentSaveData',
    );
    ...
    try {
      ...
      this.logger.debug(
        `Finished saving batches. Total saved: ${totalSavedCount}, total skipped: ${totalSkippedCount}`,
        'StudentSaveData',
      );
        ...
        await Promise.all(
          emailBatch.map(([email, password]) =>
            sendInitialPasswordEmail(email, password).catch((err) => {
              this.logger.error(
                `Error sending password email to ${email}: ${err.message}`,
                'StudentSaveData',
              );
            }),
          ),
